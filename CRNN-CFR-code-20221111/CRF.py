...
class CNNLayer(nn.Module):
    def __init__(self):
        ...
        self.conv = torch.nn.Sequential(
            # 输出尺寸（batch_size, out_channels, seq_len, 1）
...
def make_tensors1(data):
    ...
    targnum = (~np.isin(targ, np.array([8]))).sum(axis=1)
    ...
    targidx = ~np.isin(targlc, np.array([8]))
...
bnixu = [False, True]
bdir = [r'.\parameter\modelpara_forward.pth', r'.\parameter\modelpara_reversed.pth']
